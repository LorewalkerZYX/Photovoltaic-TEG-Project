...
import xlsxwriter
import LoadDataS_TEG
import SetSeed
import Normal_TEG as Normal
...
# Preparing the data
dev_ratio = 0.1
Train_x, Train_y, test_x, test_y = SplitData.split(X_train, Y_train, dev_ratio)

trainx, trainy, validx, validy = SplitData.split(Train_x, Train_y, dev_ratio)
'''
[trainx, trainy, validx, validy, test_x, test_y] = LoadDataS_TEG.LoadData()
...
net = Net(8, hidden_feature, 2, hidden_layers)
net = net.to(device)
...
# save in excel
workbook = xlsxwriter.Workbook('result_N%dL%dSeed=42_ln1.xlsx' %
...
worksheet.write('E1', 'Test Tpv Data')
worksheet.write('F1', 'Predict Power Data')
worksheet.write('G1', 'Predict Tpv Data')
worksheet.write('H1', 'Power Relative error')
worksheet.write('I1', 'Tpv Relative error')
worksheet.write('J1', 'Power Average Relative error')
worksheet.write('K1', 'Tpv Average Relative error')
...
# print(temp1)
workbook.close()
torch.save(net.state_dict(), 'TEG_test0.pkl')
